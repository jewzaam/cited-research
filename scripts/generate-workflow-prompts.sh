...
# Generates individual cited-research prompt files for each workflow tool.
# Usage: ./generate-workflow-prompts.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROMPT_DIR="${SCRIPT_DIR}/../prompts/workflow-tools"
TEMPLATE_SOURCE="/home/nmalik/source/personal-assistant-work/scratch/workflow-cited-research.md"
...
# Extract the prompt template section (between the --- delimiters after "## Prompt Template")
# and the comparison sheet data
for tool in "${TOOLS[@]}"; do
    lower=$(echo "${tool}" | tr '[:upper:]' '[:lower:]')
    outfile="${PROMPT_DIR}/${lower}.md"

    cat > "${outfile}" << PROMPT_EOF
Research \`${tool}\` as a workflow orchestration platform for a personal AI-assisted engineering assistant. Produce a citation-backed analysis.
...
I already evaluated n8n hands-on. The analysis at ~/source/cited-research/research/n8n/ is the benchmark for depth and structure. I also have an internal comparison spreadsheet (stale, use as context not authority) that evaluated: Temporal, ActivePieces, Serverless Workflow, Argo Workflows, Cadence, Windmill, Kestra, Prefect, n8n, Hatchet, Workato, Apache Airflow. Key data from that sheet for \`${tool}\`:

**From internal comparison (stale — verify against current sources):**
- Check the sheet data provided below for \`${tool}\`'s row entries on: HA, integrations, license, hosted/local, durable execution, MCP integration, REST API, deployment, secrets, pauseability, primary use case, workflow language, debugging, logging.
...
echo ""
echo "All prompt files created in ${PROMPT_DIR}/"
